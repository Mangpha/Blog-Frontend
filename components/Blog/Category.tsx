...
export const Category: React.FC<ICategoryProps> = ({ data, selectId }) => {
  ...
  return (
    <div className="text-lg font-medium text-center text-gray-500 dark:text-gray-400">
      ...
      <Link href={`/blog`}>
        <a className={`inline-block text-xl font-bold mx-2 rounded-lg hover:text-gray-900 dark:hover:text-white transition-colors`}>All Posts</a>
      </Link>
          ...
          <Link href={`/blog/category/${category.id}`} key={category.id}>
            <a
              className={`inline-block ${
                selectId === category.id && 'dark:text-white'
              } border-2 py-1 px-2 rounded-lg hover:text-violet-300 dark:hover:text-gray-200 transition-colors mx-2`}
            >
              {category.name}
            </a>
          </Link>
